...
public class SlackWebhookPayloadBuilderTests
{
    ...
    public void Build_With_All_Possible_Blocks_Returns_Correct_SlackWebhookPayload()
    {
        ...
        var builder = new SlackWebhookPayloadBuilder()
            ...
            .AddFileBlock(f => f.WithExternalId("external-unique-id").WithSource("source"))
            .AddHeaderBlock(h => h.WithText(new TextObject { Type = TextObjectType.PlainText, Text = "Header Text" }))
            .AddInputBlock(i =>
            ...
            .AddRichTextBlock(r => r.AddElement(() => new RichTextSection
                { Elements = new[] { new ChannelElement { ChannelId = "123" } } }))
            .AddVideoBlock(v =>
                v.WithAltText("Alt Text")
                    .WithTitle(to => to.WithType(TextObjectType.PlainText)
                        .WithText("Text"))
